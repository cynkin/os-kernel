...
void pic_init() {
  ...
  log_debug("  Saved Master PIC mask: 0x%x, Slave PIC mask: 0x%x", mask1,
            mask2);

  // Start initialization sequence (ICW1)
  log_debug("  Sending ICW1 to Master PIC (0x%x) and Slave PIC (0x%x)",
            ICW1_INIT, ICW1_INIT);
  ...
  outb(PIC1_DATA,
       0x04); // Tell Master PIC there is a Slave PIC at IRQ2 (0000 0100)
  io_wait();
  ...
  log_debug("  Restored Master PIC mask: 0x%x, Slave PIC mask: 0x%x", mask1,
            mask2);
